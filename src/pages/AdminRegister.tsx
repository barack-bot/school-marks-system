...
const getMaxAdminLimit = () => MAX_ADMINS;
...
export const AdminRegister = () => {
  ...
  const checkAdminLimit = async () => {
    try {
      ...
      setAdminCount(count);
      setCanRegister(count < MAX_ADMINS);
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!canRegister) {
      setError(
        `Maximum admin limit (3) has been reached. Currently: ${adminCount} admins. No more admins can be registered.`,
      );
      return;
    }
    ...
    try {
      ...
      // Create admin profile immediately (even if email not confirmed yet)
      ...
      // Check if email is confirmed - required for admin login
      const emailConfirmed = data?.user?.email_confirmed_at;
      if (!emailConfirmed) {
        setSuccess(
          "✅ Registration successful! Please check your email to verify your account. You'll need to confirm your email before you can login.",
        );
        // Clear form
